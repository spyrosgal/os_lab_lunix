                             ...
                             struct page **res_page)
{
	...
	*res_page = NULL;
	...
	for (i=0; i < npages; i++) {
		...
		lb = ext2_last_byte(dir, i);
		ext2_debug("kaddr=%llu, lb=%u\n", kaddr, lb);

		de = (ext2_dirent *) kaddr;
		kaddr += lb;
		ext2_debug("Initial de: %llu\n", de);
		while((char *) de < kaddr) {
			ext2_debug("current name: %c\n", *(de->name));
			if(ext2_match(namelen, name, de)) {
				// Do we need ext2_put_page() here? Probably not.
				*res_page = page;
				ext2_debug("de: %llu\n", de);
				return de;
			}
