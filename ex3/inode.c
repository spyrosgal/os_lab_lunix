                                         ...
                                         struct buffer_head **p)
{
	...
	*p = NULL;
	...
	/* ? */
	block_group = (ino - 1) / inodes_pg; // ino - 1 because inodes are 1-indexed
	ext2_debug("block_group: %lu\n", block_group);
...
static int ext2_do_write_inode(struct inode *inode, int do_sync)
{
	...
	struct buffer_head *bh;
	ext2_debug("Entered here\n");
	struct ext2_inode *raw_inode = ext2_get_inode(sb, ino, &bh);
	ext2_debug("received raw inode: %llu\n", raw_inode);
	int n;
	int err = 0;
